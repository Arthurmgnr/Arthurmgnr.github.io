 ...
 * Tous les états globaux au même endroit pour meilleure clarté et maintenabilité
 */
...
const AppState = {
  ...
  /**
   * Initialise l'état à partir du sessionStorage et localStorage
   */
  init() {
    const sessionLeaveState = sessionStorage.getItem("leaveState");
    if (sessionLeaveState) {
      this.leaveState = JSON.parse(sessionLeaveState);
    }
  },
  ...
  /**
   * Sauvegarde l'état du leaveState en sessionStorage
   */
  persistLeaveState() {
    sessionStorage.setItem("leaveState", JSON.stringify(this.leaveState));
  },
