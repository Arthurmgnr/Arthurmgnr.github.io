...
class CalendarManager {
    ...
    renderDay(day) {
        ...
        if (AppState.ui.isSchoolHolidaysActive) {
            ...
            if (SchoolHolidays.SCHOOL_HOLIDAYS[vacYearKey]) {
                const ranges = [
                    ...SchoolHolidays.SCHOOL_HOLIDAYS[vacYearKey][AppState.ui.schoolHolidaysArea],
                    ...SchoolHolidays.SCHOOL_HOLIDAYS[vacYearKey].commun,
                ];
        ...
        if (!AppState.ui.isLeaveActive) {
            ...
            if (newTotal > oldTotal && category.disponibles > 0) {
                if (newTotal > category.disponibles) {
                    ...
                    this.updateDay(dateKey);
                    updateSummaryFromCalendar();
                    Toasts.addToast("error-not-enough-days", { congeActif: category.label });
                    return;
                }
                if (newTotal === category.disponibles) {
                    Toasts.addToast("info-reached-days", { congeActif: category.label });
                }
            }
        }
