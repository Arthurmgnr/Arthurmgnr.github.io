...
export function createFilterCategories(categorie, titleElement, titleItem, items, onChange) {
    ...
    filterGroupElement.getSelected = function(value) {
        if (filtersActif.length === 0) {
            ...
            return !filtersActif.some(item => value.replaceAll("'", "").replaceAll("-", "").split(",").includes(item));
        }
    }
